...
class MultiHeadAttention(nn.Moudle):
    ...
    def forward(self, x, mask=None):
        B, L, h = x.size() # B: batch size, L: sequence length, h: n_embed

        # calculate query key values for all heads in batch 
        q, k, v = self.q(x), self.k(x), self.v(x)
        # size after transpose: B, n_heads, L, n_embed
        k = k.view(B, L, self.n_heads, h//self.n_heads).transpose(1, 2)
        q = q.view(B, L, self.n_heads, h//self.n_heads).transpose(1, 2)
        v = v.view(B, L, self.n_heads, h//self.n_heads).transpose(1, 2)
...
class PositionalEncoding(nn.Module):
    def __init__(self, max_pos, n_embed):
        super(PositionalEncoding, self).__init__()
        # 初始化位置编码矩阵
        PE = torch.zeros(max_pos, n_embed)
        pos = torch.arrange(0, max_pos, dtype=torch.float).unsqueeze(1)
        multiterm = torch.arange(0, n_embed, 2, dtype=torch.float)
        # 计算pos中对应的系数部分
        # e^(2i*(-log(10000.0)/d))
        # 他从数学上等价于1/10000^(2i/d)
        multiterm = torch.exp(multiterm * (-math.log(10000.0) / n_embed))
        PE[:, 0::2] = torch.sin(pos * multierm)
        PE[:, 1::2] = torch.cos(pos * multierm)
